...
export async function checkPath(result: any) {
    ...
    result
        ...
        .forEach(({ path, filePath }: any) => {
            ...
            // 自身路由配置是否冲突
            const exit = result.find(({ filePath: f, path: p }: any) => {
 ...
 * @returns {boolean}
 */
export function isNoAuthPage(pathname?: string) {
